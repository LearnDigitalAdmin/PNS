...
export default function ServicesPage() {
  const { openModal, services, servicesLoading, testimonials, testimonialsLoading } = useSite();
  const looped = services.length > 0 ? [...services, ...services] : [];
  ...
  return (
    <div className="px-5 md:px-10 py-7">
      ...
      {servicesLoading ? (
        ...
        <p className="max-w-7xl mx-auto text-xs" style={{ color: 'var(--mid-gray)' }}>
          Our service list is being updated — check back soon.
        </p>
      ) : (
            ...
            <div className="glass p-5" style={{ borderColor: 'rgba(0,0,0,.08)' }} key={t.id}>
              ...
              <div className="mt-3 flex items-center gap-3">
                {t.image ? (
                  ...
                  <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'var(--gold-dim)', color: '#9a7a2c', fontWeight: 700 }}>
                    {t.name?.[0] ?? '?'}
                  </div>
                )}
                <div>
