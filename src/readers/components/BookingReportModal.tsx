...
import { BOOKING_REPORT_REASONS } from '../../partners/types';

// Booking-specific counterpart to src/site/components/ReportModal.tsx — kept
// as a separate component rather than a shared one because this report is
// always tied to a signed-in reader (reporterUserId is required, never
// anonymous — see firestore.rules) and uses its own reason list.
export default function BookingReportModal({
  bookingId,
  photographerId,
  reporterUserId,
  onClose,
  ...
  const [reason, setReason] = useState<string>(BOOKING_REPORT_REASONS[0]);
